...
// Pool-level (not per-loan) approach: tracks outstanding balance, monthly
// interest income, origination fees, cost of funds, default losses.
...
export function buildFinancing(a: Assumptions): FinancingResult {
  ...
  for (let y = 0; y < a.years; y++) {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      // Interest accrues on average balance after disbursement; principal
      // repays straight-line over `term` months.

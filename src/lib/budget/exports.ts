// Board pack exports: Excel (xlsx), PDF (jspdf), PPTX (pptxgenjs).
// All exports are client-side; no server roundtrip.
  ...
  XLSX.utils.book_append_sheet(
    ...
    XLSX.utils.json_to_sheet(
      ctx.statements.balanceSheet.map((r) => ({
        ...
        Month: r.month,
        Cash: r.cash,
        AR: r.accountsReceivable,
        "Fixed assets": r.fixedAssets,
        "Total assets": r.totalAssets,
        AP: r.accountsPayable,
        Debt: r.debt,
        "Total liabilities": r.totalLiabilities,
        Equity: r.equity,
        "L + E": r.totalLiabEquity,
        Check: r.check,
      })),
    ),
    "Balance Sheet",
  ...
  XLSX.utils.book_append_sheet(
    ...
    XLSX.utils.json_to_sheet(
      ctx.model.monthly.map((m) => ({
        Year: m.year,
        Month: m.month,
        "Starting cust": m.startingCustomers,
        New: m.newCustomers,
        Churned: m.churnedCustomers,
        "Ending cust": m.endingCustomers,
        Income: m.totalIncome,
        Cost: m.totalCost,
        EBITDA: m.ebitda,
        "Stream rev": m.streamIncome,
        "Stream cost": m.streamCost,
        "Financing in": m.financingIncome,
        "Financing cost": m.financingCost,
...
export function exportPDF(ctx: BoardPackContext): void {
  ...
  doc.setFontSize(20);
  doc.text("Nordenergi · Board Pack", 40, 50);
  doc.setFontSize(11);
  doc.text(
  ...
  autoTable(doc, {
    ...
    body: ctx.model.yearly.map((y) => [
      y.year,
      fmt(y.endingCustomers),
      fmtCompact(y.totalIncome),
      fmtCompact(y.ebitda),
      fmtCompact(y.cashFlow),
      fmtCompact(y.streamIncome),
      fmtCompact(y.financingEndingOutstanding),
    ]),
    styles: { fontSize: 9 },
    headStyles: { fillColor: [30, 41, 59], textColor: 255 },
    didDrawPage: () => {
      ...
      doc.text("Nordenergi · Board Pack · Confidential", 40, doc.internal.pageSize.getHeight() - 20);
    },
  });
  ...
  doc.addPage();
  doc.setFontSize(14);
  doc.text("Annual Cash Flow", 40, 40);
  const cfByYear = aggregateByYear(ctx.statements.cashFlow, [
    "netIncome", "depreciation", "changeAR", "changeAP", "cfo", "capex", "cfi", "debtChange", "cff", "netChange",
  ]);
  autoTable(doc, {
    startY: 60,
    head: [["Year", "Net income", "D&A", "Δ AR", "Δ AP", "CFO", "Capex", "CFI", "Debt Δ", "CFF", "Net change"]],
    body: cfByYear.map((r) => [
      r.year,
      ...["netIncome", "depreciation", "changeAR", "changeAP", "cfo", "capex", "cfi", "debtChange", "cff", "netChange"]
        .map((k) => fmtCompact((r as Record<string, number | string>)[k] as number)),
...
export async function exportPPTX(ctx: BoardPackContext): Promise<void> {
  ...
  pptx.title = `Nordenergi Board Pack — ${ctx.scenarioName}`;
  ...
  s1.addText("NORDENERGI", {
    x: 0.6, y: 2.6, w: 12, h: 0.6,
    color: "FFFFFF", fontFace: "Arial", fontSize: 36, bold: true, charSpacing: 12,
  });
  s1.addText("Board Pack · 10-year financial plan", {
    x: 0.6, y: 3.4, w: 12, h: 0.5, color: ACCENT, fontFace: "Arial", fontSize: 20,
  });
  s1.addText(
    `Scenario: ${ctx.scenarioName}   ·   ${ctx.generatedAt.toLocaleDateString("sv-SE")}`,
  ...
  kpis.forEach(([label, val], i) => {
    ...
    s2.addShape(pptx.ShapeType.rect, {
      x, y: 2.0, w: 2.9, h: 2.6, fill: { color: "F1F5F9" }, line: { color: "E2E8F0", width: 1 },
    });
    s2.addText(label, {
      x: x + 0.2, y: 2.2, w: 2.5, h: 0.4,
      color: MUTED, fontFace: "Arial", fontSize: 12, bold: true, charSpacing: 8,
    });
    s2.addText(val, {
      x: x + 0.2, y: 2.8, w: 2.5, h: 1.6,
      color: TEXT, fontFace: "Arial", fontSize: 36, bold: true,
    });
  });
  ...
  s3.addChart(pptx.ChartType.bar, [
    {
      name: "Revenue",
      labels: ctx.model.yearly.map((y) => String(y.year)),
      values: ctx.model.yearly.map((y) => Math.round(y.totalIncome)),
    },
    {
      name: "EBITDA",
      labels: ctx.model.yearly.map((y) => String(y.year)),
      values: ctx.model.yearly.map((y) => Math.round(y.ebitda)),
    },
  ], {
  ...
  slideHeader(s4, "Customer base · ending units", ctx.scenarioName, NAVY, MUTED);
  s4.addChart(pptx.ChartType.line, [
    {
      name: "Customers",
      labels: ctx.model.yearly.map((y) => String(y.year)),
      values: ctx.model.yearly.map((y) => y.endingCustomers),
    },
  ], {
    x: 0.6, y: 1.5, w: 12, h: 5.5,
    chartColors: [ACCENT],
  ...
  slideHeader(s5, "Yearly summary", ctx.scenarioName, NAVY, MUTED);
  const head = [
    ["Year", "Customers", "Revenue", "EBITDA", "Cash flow", "Streams", "Financing out."].map((t) => ({
      text: t,
      options: { bold: true, color: "FFFFFF", fill: { color: NAVY }, fontSize: 11 },
    })),
  ];
  const body = ctx.model.yearly.map((y) => [
    String(y.year),
    fmt(y.endingCustomers),
    fmtCompact(y.totalIncome),
    fmtCompact(y.ebitda),
    ...
    fmtCompact(y.streamIncome),
    fmtCompact(y.financingEndingOutstanding),
  ].map((t) => ({ text: t, options: { fontSize: 10, color: TEXT } })));
  s5.addTable([...head, ...body], {
    x: 0.6, y: 1.5, w: 12, colW: [1.2, 1.6, 1.9, 1.9, 1.9, 1.7, 1.8],
...
function slideHeader(
  ...
  slide.addText(title, {
    ...
    color: navy, fontFace: "Arial", fontSize: 26, bold: true,
  });
  slide.addText(`Scenario · ${scenario}`, {
...
function fileName(ctx: BoardPackContext, ext: string): string {
  ...
  return `nordenergi-board-pack-${safe}-${date}.${ext}`;
}

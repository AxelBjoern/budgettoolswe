// Sensitivity / tornado analysis.
// For each driver, clone assumptions, apply a ±delta perturbation across all
// years, recompute the 10-year horizon and measure impact on total EBITDA.

import { compute } from "./engine";
import type { Assumptions, ComputedModel, StreamKey, YearAssumptions } from "./types";

export type DriverKey =
  | "pricePerKwh"
  | "costPerKwh"
  | "churnRate"
  | "newCustomersVolume"
  | "kwhPerCustomer"
  | "subscriptionPerCustomer"
  | "acquisitionCost"
  | "salaryCost"
  | "salesAppreciation"
  | "financingAPR"
  | "financingOriginations"
  | "streamSolar"
  | "streamBattery"
  | "streamVPP"
  | "streamSaaS";

export interface DriverDef {
  key: DriverKey;
  label: string;
  group: "Pricing" | "Volume" | "Costs" | "Financing" | "Streams";
  description: string;
}

export const DRIVERS: DriverDef[] = [
  { key: "pricePerKwh", label: "Sales price /kWh", group: "Pricing", description: "Per-kWh sell price across all years" },
  { key: "costPerKwh", label: "Purchase cost /kWh", group: "Costs", description: "Direct kWh purchase cost" },
  { key: "subscriptionPerCustomer", label: "Subscription fee", group: "Pricing", description: "Annual subscription per customer" },
  { key: "salesAppreciation", label: "Sales appreciation", group: "Pricing", description: "Year-on-year price escalation" },
  { key: "newCustomersVolume", label: "New customer volume", group: "Volume", description: "Acquired customers / year" },
  { key: "churnRate", label: "Churn rate", group: "Volume", description: "Annual customer churn" },
  { key: "kwhPerCustomer", label: "kWh / customer", group: "Volume", description: "Avg yearly consumption per customer" },
  { key: "acquisitionCost", label: "Acquisition cost", group: "Costs", description: "Sales cost per acquired customer" },
  { key: "salaryCost", label: "Salary cost", group: "Costs", description: "Personnel cost" },
  { key: "financingAPR", label: "Financing APR", group: "Financing", description: "Customer interest rate" },
  { key: "financingOriginations", label: "Financing originations", group: "Financing", description: "New loans/leases per year" },
  { key: "streamSolar", label: "Solar units / yr", group: "Streams", description: "New solar installations per year" },
  { key: "streamBattery", label: "Battery units / yr", group: "Streams", description: "New battery systems per year" },
  { key: "streamVPP", label: "VPP units / yr", group: "Streams", description: "New VPP enrollments per year" },
  { key: "streamSaaS", label: "SaaS subs / yr", group: "Streams", description: "New SaaS subscribers per year" },
];

const STREAM_MAP: Record<string, StreamKey> = {
  streamSolar: "solar",
  streamBattery: "battery",
  streamVPP: "vpp",
  streamSaaS: "saas",
};

function applyDriver(a: Assumptions, key: DriverKey, factor: number): Assumptions {
  const next: Assumptions = structuredClone(a);
  const mut = (fn: (y: YearAssumptions) => void) => next.perYear.forEach(fn);

  switch (key) {
    case "pricePerKwh":
      mut((y) => { y.pricePerKwh *= factor; });
      break;
    case "costPerKwh":
      mut((y) => { y.costPerKwh *= factor; });
      break;
    case "subscriptionPerCustomer":
      mut((y) => { y.subscriptionPerCustomerYear *= factor; });
      break;
    case "salesAppreciation":
      next.salesAppreciationPct = (next.salesAppreciationPct ?? 0) * factor;
      break;
    case "newCustomersVolume":
      mut((y) => {
        for (const k of Object.keys(y.newCustomersByChannel) as Array<keyof typeof y.newCustomersByChannel>) {
          y.newCustomersByChannel[k] = Math.round(y.newCustomersByChannel[k] * factor);
        }
      });
      break;
    case "churnRate":
      mut((y) => { y.churnRate = Math.min(0.99, Math.max(0, y.churnRate * factor)); });
      break;
    case "kwhPerCustomer":
      mut((y) => { y.kwhPerCustomerYear *= factor; });
      break;
    case "acquisitionCost":
      mut((y) => { y.acquisitionCostPerCustomer *= factor; });
      break;
    case "salaryCost":
      mut((y) => {
        y.salaries = y.salaries.map((r) => ({ ...r, monthlySalary: r.monthlySalary * factor }));
      });
      break;
    case "financingAPR":
      if (next.financing) next.financing.customerAPR *= factor;
      break;
    case "financingOriginations":
      if (next.financing) {
        next.financing.originationsPerYear = next.financing.originationsPerYear.map((o) => Math.round(o * factor));
      }
      break;
    case "streamSolar":
    case "streamBattery":
    case "streamVPP":
    case "streamSaaS": {
      const sk = STREAM_MAP[key];
      mut((y) => {
        const s = y.streams?.[sk];
        if (s) s.newUnitsPerYear = Math.round(s.newUnitsPerYear * factor);
      });
      break;
    }
  }
  return next;
}

function totalEbitda(m: ComputedModel): number {
  return m.yearly.reduce((a, y) => a + y.ebitda, 0);
}

export interface SensitivityRow {
  key: DriverKey;
  label: string;
  group: DriverDef["group"];
  base: number;
  low: number;
  high: number;
  lowDelta: number;
  highDelta: number;
  spread: number;
}

export interface SensitivityResult {
  baseEbitda: number;
  deltaPct: number;
  rows: SensitivityRow[];
}

/**
 * Build a tornado on total horizon EBITDA. `deltaPct` is the absolute
 * perturbation applied to each driver (e.g. 0.1 → ±10%).
 */
export function buildSensitivity(a: Assumptions, deltaPct = 0.1): SensitivityResult {
  const base = totalEbitda(compute(a));
  const rows: SensitivityRow[] = DRIVERS.map((d) => {
    const low = totalEbitda(compute(applyDriver(a, d.key, 1 - deltaPct)));
    const high = totalEbitda(compute(applyDriver(a, d.key, 1 + deltaPct)));
    const lowDelta = low - base;
    const highDelta = high - base;
    return {
      key: d.key,
      label: d.label,
      group: d.group,
      base,
      low,
      high,
      lowDelta,
      highDelta,
      spread: Math.abs(highDelta - lowDelta),
    };
  }).sort((a, b) => b.spread - a.spread);

  return { baseEbitda: base, deltaPct, rows };
}
